...
from .constants import TRANSACTION_TYPE,DEPOSIT,WITHDRAW,LOAN,LOAN_PAID,TRANSFER
...
class TransferMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        amount=form.cleaned_data.get('amount')
        receiver_account_number = form.cleaned_data.get('receiver_account')
        
        try:
            receiver_account = UserBankAccount.objects.get(account_no=receiver_account_number)
        except UserBankAccount.DoesNotExist:
            form.add_error('receiver_account', 'The account does not exist.')
        ...
        messages.success(self.request,f"{amount}tk was transferred successfully")
        send_transaction_email(self.request.user, amount, "Send Money Message", "transactions/send_money_email.html")
        send_transaction_email(receiver_account.user, amount, "Receive Money Message", "transactions/receive_money_email.html")
        return super().form_valid(form)
